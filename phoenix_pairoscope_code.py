...
# code is MIT license from phoenix pipeline
# see https://github.com/tgen/phoenix/blob/281cb80221a0dd5ccabac39a326b3a7ff1f1fb19/required_scripts/mm_igtx_pairoscope_calling_b38_356362b.py
# adapted to hg19 back
...
def call_translocations(sample, gene, gene_chr, order, window_start=0, window_end=300000000):
    ...
    if order == "standard":
        ...
        if gene == 'MYC':
            ...
            gene_igh = dreads[(dreads.Specimen == sample) & (dreads.ChrA == gene_chr) & (dreads.ChrB == "14")
                              & (dreads.PositionA >= window_start) & (dreads.PositionA <= window_end)
                              & ((dreads.PositionA <= 129571461) | (dreads.PositionA >= 129571635))]
            count_gene_igh = len(gene_igh.index)
            print(str(gene_igh.columns))
            print(str(gene_igh))
            gene_igk = dreads[(dreads.Specimen == sample) & (dreads.ChrA == "2") & (dreads.ChrB == gene_chr)
                              & (dreads.PositionB >= window_start) & (dreads.PositionB <= window_end)
                              & ((dreads.PositionB <= 129571461) | (dreads.PositionB >= 129571635))]
            count_gene_igk = len(gene_igk.index)
            print(str(gene_igk.columns))
            print(str(gene_igk))
            gene_igl = dreads[(dreads.Specimen == sample) & (dreads.ChrA == gene_chr) & (dreads.ChrB == "22")
                              & (dreads.PositionA >= window_start) & (dreads.PositionA <= window_end)
                              & ((dreads.PositionA <= 129571461) | (dreads.PositionA >= 129571635))]
...
#                              & ((dreads.PositionA <= 143243200) | (dreads.PositionA >= 143244000))]
        ...
        gene_igh = dreads[(dreads.Specimen == sample) & (dreads.ChrA == "14") & (dreads.ChrB == gene_chr)
                          & (dreads.PositionB >= window_start) & (dreads.PositionB <= window_end)]
        count_gene_igh = len(gene_igh.index)
        gene_igk = dreads[(dreads.Specimen == sample) & (dreads.ChrA == "2") & (dreads.ChrB == gene_chr)
                          & (dreads.PositionB >= window_start) & (dreads.PositionB <= window_end)]
...
def make_sample_calls(sample):
    ...
    # MMSET
    mmset_call = call_translocations(gene="NSD2", gene_chr="4", order="standard", sample=sample_id)

    # CCND3
    ccnd3_call = call_translocations(gene="CCND3", gene_chr="6", order="standard", sample=sample_id)

    # MYC
# TODO search for blacklisted regions around these genes in hg19
...
#                                   window_end=129487754, sample=sample_id)
    myc_call = call_translocations(gene="MYC", gene_chr="8", order="standard", sample=sample_id)

    # MAFA
    mafa_call = call_translocations(gene="MAFA", gene_chr="8", order="standard", sample=sample_id)
    ...
    # CCND1
    ccnd1_call = call_translocations(gene="CCND1", gene_chr="11", order="standard", sample=sample_id)

    # CCND2
    ccnd2_call = call_translocations(gene="CCND2", gene_chr="12", order="standard", sample=sample_id)

    # MAF
    maf_call = call_translocations(gene="MAF", gene_chr="16", order="reverse", sample=sample_id)

    # MAFB
    mafb_call = call_translocations(gene="MAFB", gene_chr="20", order="reverse", sample=sample_id)
